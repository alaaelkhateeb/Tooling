...
def extractFemaleNames(filename,letter):
  ...
  names=re.findall(r'<td>(\d+)</td><td>([A-Z][a-z]+)</td><td>([A-Z][a-z]+)</td>',data)
  for i,j,k in names:
    if k[0]==letter:
        temp.append(k+' '+i)
  temp.sort()
...
def extractDocAndPopularityDate(filename):
  ...
  file=open(filename,'r')
  data=file.read()
  d1=re.findall(r'<h1>Popular Names by Birth Year</h1>(.*?)</td>',data)
  d2=re.findall(r">(\w+ in \d+)</h3>",data)
  
  return str(d1[0])+'and Popularity in'+str(d2[0])
